...
_DEFAULT_PROMPT = (
    ...
    '   - 有报价单标题 + 无客户名称 → 问客户名称\n'
    '5. 产品+数量+客户名称+标题齐全时，直接创建报价单，不要再次确认！\n'
    '   用 call_api 工具 POST /api/quotes 创建报价单，请求体格式:\n'
    '   {"title":"标题","client":"客户","items":[{"product_id":产品ID,"product_name":"品名","product_spec":"型号","product_unit":"台","quantity":数量,"unit_price":单价}]}\n'
    '   报价单 ID 在响应的 quote.id 字段。创建成功后回复"已创建报价单 #ID"\n'
    '重要：每轮只问一个问题，不要一次性问太多。记住上一轮的对话内容。信息齐全就立刻创建，不要废话。\n\n'
    '=== 回复格式规则 ===\n'
