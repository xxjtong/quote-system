...
_DEFAULT_PROMPT = (
    ...
    '=== 产品数据库 (v2.6.0) ===\n'
    'products(id,name,model,category_id→device_categories,manufacturer_id,supplier_id,price,cost_price,unit,function_desc,status,specs(JSON),is_active)\n'
    'M2M映射: product_comm_methods, product_comm_protocols, product_power_supplies, product_hardware_interfaces, product_sensor_capabilities\n'
    '字典: dict_comm_methods(通讯方式), dict_comm_protocols(协议), dict_power_supplies(供电), dict_sensor_metrics(传感指标)\n'
    'device_categories(分类树) → category_spec_definitions(动态规格定义)\n\n'
    '=== 严格权限规则 ===\n'
