"""Products Blueprint — 产品CRUD、导入导出、OCR、智能识别、图片上传"""
import os
import io
import re
import random
...
from models import Product, FieldSetting
from auth import require_auth, require_admin
from utils import (
    BASE_DIR, UPLOAD_DIR, EXPORT_DIR, _debug_log, _log_ai_usage, _safe_number,
    _parse_json_reply, _product_from_parsed, _store_image_blob,
    get_field_visibility, filter_fields_for_user,
)
...
@products_bp.route('/download-image', methods=['POST'])
def download_image():
...
def doubao_vision_recognize(image_b64, mime_type='image/jpeg'):
    ...
    try:
        ...
        if parsed:
            product = _product_from_parsed(parsed, str(result))
            if product:
                return product
...
def deepseek_parse_product(text, _gateway_url):
...
def recognize_product():
    ...
    _gateway_url = current_app.config.get('GATEWAY_URL', os.environ.get('QUOTE_GATEWAY_URL', 'http://127.0.0.1:8643'))
    ...
    if uploaded_file:
        ...
        try:
            ...
            if text:
                product = deepseek_parse_product(text, _gateway_url)
                if product:
...
def export_product_template():
    ...
    if template_path.exists():
        return _send_file(str(template_path), download_name='硬件报价规格库（成本）.xlsx', as_attachment=True)
    # 兜底：生成简易模板
    import openpyxl
    from openpyxl.styles import Font, Alignment
    from openpyxl.utils import get_column_letter
    ...
    wb.save(output)
    output.seek(0)
    return _send_file(output, download_name='产品导入模板.xlsx', as_attachment=True)
