...
## v3.1.0 (2026-05-31) — 产品数据库全面优化

### 数据库优化
- 17 个缺失索引补建（产品 FK、关联表、复合索引）
- 1 个冗余索引清理
- M2M 数据智能提取：从 product_desc 解析通讯方式(343行)、协议(11行)、供电(334行)、硬件接口(123行)、传感能力(394行)
- 45 个制造商从供应商自动创建，354 产品关联

### 产品类型分类
- 新增 `dict_product_types` 字典表 + `products.product_type_id` FK
- 8 种类型：传感器/控制器/网关/路由器/配件/终端设备/执行器/电源
- 392 产品智能分类，列表增加"类型"和"功能(传感/控制)"列

### 品类多标签
- 新增 `product_categories` M2M 表，产品支持多品类标签
- 编辑页品类改为多选 pills + 手动输入
- 267 条历史数据迁移

### 产品规格爬取
- 102 个星纵产品从产品页爬取详细规格参数
- 通讯详情、供电规格、电池续航、传感精度量程等
- PDF 规格书解析（EG71）

### 图片优化
- 上传图片生成 64px 缩略图，列表加载缩略图减少带宽 12x
- 废弃 BLOB 存储，只用磁盘文件 + product_images 表
- 删除产品/替换图片时自动清理磁盘文件
- 107 破损本地图片替换为远程 URL，修复 `/uploads/` 路径认证

### 品类/类型/厂商/通讯等字段改为下拉+手动输入
- 7 个字段支持 `✏️ 手动输入...`，自动创建新字典条目
- 列表筛选支持多选 AND 逻辑（通讯方式、供电、传感功能）
- 品类改为多选标签 pills

### 导入预览
- 上传 Excel 后预览表格，选中确认再导入
- LLM 智能列识别 + 自动表头检测
- 产品类型智能分类，已存在产品默认不选
- 品类下拉/类型智能识别/厂商数据列表

### 产品图片修复
- dev 模式图片 URL 自动前缀 Flask 端口
- 详情/列表/编辑页图片显示修复
- 星纵产品图片从官网抓取更新

### 权限优化
- 产品数据库对所有登录用户开放
- 导入导出权限控制：普通用户只能操作自己的产品
- 字段可见性跟随管理设置实时生效（30s缓存）
