...
#### 前端模块化（v1.3.7）
前端从单一 `index.html`（2285行）拆分为 4 个文件：
- `templates/index.html`（480行）— HTML 结构 + CSS
- `static/js/app.js`（306行）— 全局状态、API 封装、导航、Dashboard
- `static/js/auth.js`（153行）— 登录/注册/会话管理
- `static/js/products.js`（1377行）— 产品/报价/导入/管理
...
### 5.4 测试（v1.5.5）
- pytest 测试套件：98 项，覆盖认证(13) / 产品(24) / 报价(18) / 管理(17) / 边界(22)
